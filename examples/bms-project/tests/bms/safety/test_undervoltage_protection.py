#!/usr/bin/env python3
"""
Under-Voltage Protection Test for BMS Safety Validation

Tests the BMS under-voltage protection function per ISO 26262 ASIL-C requirements.
Validates that cell under-voltage is detected within FTTI and contactor opens.

Workflow: battery-bms-validation.yaml -> safety-functions-validation job
CLI: --threshold-v 2.5 --reaction-time-ms 100 --fault-injection all-cells --output results/safety-undervoltage.json
"""
...
class UnderVoltageTestResult:
    """Container for under-voltage protection test results."""

    def __init__(self):
        self.test_id = "UV-PROT-001"
        self.test_name = "Under-Voltage Protection Validation"
        self.timestamp = datetime.utcnow().isoformat() + "Z"
        self.passed = False
        self.reaction_time_ms = 0.0
        self.ftti_requirement_ms = 100.0
...
def run_undervoltage_test(
    threshold_v: float,
    ftti_ms: float,
    fault_injection: str
) -> UnderVoltageTestResult:
    """
    Execute under-voltage protection test.

    Args:
        threshold_v: Under-voltage threshold in volts (default 2.5V)
        ftti_ms: Fault Tolerant Time Interval in milliseconds (default 100ms)
        fault_injection: Injection method - "all-cells" or "single-cell"

    Returns:
        UnderVoltageTestResult with all test metrics
    """
    result = UnderVoltageTestResult()
    ...
    limits = SafetyLimits(
        cell_ov_l2_v=4.25,
        cell_uv_l2_v=threshold_v,
        pack_oc_l2_a=500.0,
        temp_ot_l2_c=60.0
    )
    timing = TimingRequirements(
        overvoltage_detection_ms=10.0,
        undervoltage_detection_ms=ftti_ms,
    ...
    uv_fault_found = any(
        f.event_type in [SafetyEventType.UNDERVOLTAGE_L1, SafetyEventType.UNDERVOLTAGE_L2]
    ...
    if uv_fault_found:
        ...
        result.details.append({
            ...
            "uv_fault_detected": False,
            "message": "WARNING: Under-voltage fault code not found"
        })
...
def format_results(result: UnderVoltageTestResult) -> Dict[str, Any]:
    ...
    return {
        ...
        "iso_26262_compliance": {
            ...
            "safety_mechanism": "Under-voltage detection L2",
...
def main():
    parser = argparse.ArgumentParser(
        description="BMS Under-Voltage Protection Test per ISO 26262 ASIL-C"
    )
    parser.add_argument(
        "--threshold-v",
        type=float,
        default=2.5,
        help="Under-voltage threshold in volts (default: 2.5V)"
    )
    parser.add_argument(
        "--reaction-time-ms",
        type=float,
        default=100.0,
        help="Fault Tolerant Time Interval in ms (default: 100ms)"
    )
    parser.add_argument(
        "--fault-injection",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/safety-undervoltage.json",
        help="Output JSON file path (default: results/safety-undervoltage.json)"
    )

    args = parser.parse_args()

    # Run test
    print(f"Running Under-Voltage Protection Test...")
